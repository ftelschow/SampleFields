...
GaussDensitySum2DNoise <- function(N,
                                   ...
                                   sigma = function(x) array(outer( x, x, FUN = function(s,t) (s+1)/(t^2+1) )/3, c(rep(length(x),2), 1)), randNumber=rnorm, M=6, bdx=0.02, Fnorm=NULL){
  if(is.null(Fnorm)){
